...
object Naive {
  val insertSQL =
    """
      |WITH raw_data (asset_id, measure_point_id, the_timestamp, the_value) AS ( VALUES (?, ?, ?, ?))
  ...
  def run(connection: Connection, numBatches: Int, batchSize: Int): Unit = {
    ...
    for (_ <- 1 to numBatches) {
      for (_ <- 1 to batchSize) {
        preparedStatement.setInt(1, Random.between(1, 120))
        preparedStatement.setInt(2, Random.between(1, 120))
        preparedStatement.setLong(3, System.nanoTime())
